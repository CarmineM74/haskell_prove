...
tournamentOutcome t = sum [ outcome a b | (a,b) <- matches]
    where
        a_moves = fst t
        b_moves = snd t
        matches = zip a_moves b_moves
...
sLostLast move = sConst $ lose move

sWonLast :: Move -> Strategy
sWonLast move = sConst $ beat move
...
sLastTwo :: Strategy
sLastTwo (m1:m2:_)
    | m1 /= m2 = randomStrategy []
    | otherwise =  case moves_outcome remaining_moves of
                            1 -> head remaining_moves
                            -1 -> last remaining_moves
        where
            moves_outcome (h:t:_) = outcome h t
            remaining_moves = [ m | m <- [Rock,Paper,Scissors], m /= m1 ]

computeMoveFreq :: [Move] -> [(Move,Int)]
computeMoveFreq [] = []
computeMoveFreq ms = [(m,countMove m) | m <- [Rock,Paper,Scissors]]
        where
            countMove mv = length [ m | m <- ms, m == mv ]

lfm :: (Move,Int) -> [(Move, Int)] -> Move
lfm (m,f) [] = m
lfm (m1,f1) ((m2,f2):ms)
    | f1 < f2 = lfm (m1,f1) ms
    | otherwise = lfm (m2,f2) ms
            
leastFrequentMove :: [(Move,Int)] -> Move
leastFrequentMove (m:ms) = lfm m ms
    
sFrequencies :: Strategy
sFrequencies moves = leastFrequentMove freqs
    where
        freqs = computeMoveFreq moves
